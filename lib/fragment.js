...
module.exports = class Fragment extends EventEmitter {
    ...
    fetch(request, isFallback = false, parentSpan = null) {
        ...
        if (isAsync) {
            return Buffer.from(
                `<script data-pipe>${this.pipeInstanceName}.placeholder(${this.index})</script>`
            );
        }
        return this.stream;
    }
    ...
    insertStart() {
        ...
        if (this.scriptRefs.length === 0) {
            this.stream.write(
                `<script data-pipe>${this.pipeInstanceName}.start(${this.index})</script>`
            );
            this.index++;
            return;
        }
        ...
        this.scriptRefs.forEach(uri => {
            this.stream.write(
                `<script data-pipe>${this.pipeInstanceName}.start(${
                    this.index
