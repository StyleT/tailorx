...
module.exports = class Transform {
    constructor(handleTags) {
        this.handleTags = handleTags;
        this.parse = memoize(parse5.parse.bind(null), { length: 1, max: 20 });
        this.parseFragment = memoize(parse5.parseFragment.bind(null), {
            length: 1,
            max: 20
        });
    }

    setHandleTags(handleTags) {
        this.handleTags = handleTags;
        return this;
    }
    ...
    applyTransforms(baseTemplate, childTemplate, fullRendering) {
        ...
        const rootNodes = fullRendering
            ? this.parse(baseTemplate, options)
            : this.parseFragment(baseTemplate, options);
        const slotMap =
