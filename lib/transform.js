...
module.exports = class Transform {
     ...
     *
     * @param {String} baseTemplate - Base template that contains all the necessary tags and fragments for the given page (Used by multiple pages)
     * @returns {Array} Array consiting of a template without ignored parts as a String and ignored parts as an Array
     */
    _ignoreBeforeParsing(baseTemplate) {
        ...
        const template = baseTemplate.replace(
            ...
            match => {
                const ignoredPartIndex = ignoredParts.push(match);
                return `<!-- TailorX: Ignored content during parsing #${ignoredPartIndex} -->`;
            }
        );

        return [template, ignoredParts];
    }
